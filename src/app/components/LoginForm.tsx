...
// Include every provider you actually enabled in Supabase > Authentication > Providers
type Provider = "google" | "twitch" | "discord" | "twitter";
...
export default function LoginForm() {
  ...
  const handleLogin = async (provider: Provider, redirectPath?: string) => {
    setIsLoading(true);
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider,
        options: {
          // Must match the whitelisted redirect URL exactly
          redirectTo: `${window.location.origin}/auth/callback${
            redirectPath ? `?redirectTo=${encodeURIComponent(redirectPath)}` : ""
          }`,
        },
      });
      ...
      >
        <div className="p-8">
          ...
          <p className="text-center text-xs text-cyan-100/40">
            ...
            <button className="text-cyan-400/60 hover:text-cyan-400 transition-colors underline">Terms of Service</button>{" "}
            and{" "}
            <button className="text-cyan-400/60 hover:text-cyan-400 transition-colors underline">Privacy Policy</button>
          </p>
        </div>
      </motion.div>
    </div>
